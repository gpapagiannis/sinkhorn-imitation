"""
The code in this file is based on the implementation of: https://github.com/Khrylx/PyTorch-RL.
"""
...
def collect_samples(pid, queue, env, policy, custom_reward,
                    mean_action, render, running_state, min_batch_size ,randomise=False):
    torch.randn(pid)
    log = dict()
    memory = Memory()
    num_steps = 0
...
def merge_log(log_list):
    ...
    es = []
    for x in log_list:
        es = es + x['episodic_rewards']
    log['episodic_rewards'] = es
...
class Agent:
    ...
    def collect_samples(self, min_batch_size, randomise=False):
        t_start = time.time()
        ...
        memory, log = collect_samples(0, None, self.env, self.policy, self.custom_reward, self.mean_action,
                                      self.render, self.running_state, thread_batch_size, randomise=randomise)
