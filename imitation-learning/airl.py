...
parser = argparse.ArgumentParser(description='Adversarial Inverse Reinforcement Learning')
...
def airl():
    ...
    if args.resume_training ==True:
        R_avg  =  pickle.load(open('experiment-logs-airl/{}/{}/avg_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        R_max  =  pickle.load(open('experiment-logs-airl/{}/{}/max_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        R_min  =  pickle.load(open('experiment-logs-airl/{}/{}/min_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        episodic_rewards =  pickle.load(open('experiment-logs-airl/{}/{}/episodic_rewards_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), "rb"))
    ...
    for i_iter in range(args.max_iter_num):
        ...
        t0 = time.time()
        num_of_samples =  airl_step(batch, i_iter)
        t1 = time.time()
        ...
        if args.save_model_interval > 0 and (i_iter) % args.save_model_interval == 0:
            ...
            pickle.dump((policy_net, value_net, discriminator, running_state), open(os.path.join(assets_dir(), 'learned_models/AIRL/{}/{}_AIRL_s{}.p'.format(args.dataset_size, args.env_name, args.seed)), 'wb'))
            to_device(device, policy_net, value_net)
            pickle.dump(R_avg, open('experiment-logs-airl/{}/{}/avg_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
            pickle.dump(R_max, open('experiment-logs-airl/{}/{}/max_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
            pickle.dump(R_min, open('experiment-logs-airl/{}/{}/min_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
            pickle.dump(episodic_rewards, open('experiment-logs-airl/{}/{}/episodic_rewards_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
