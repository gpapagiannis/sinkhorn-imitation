...
if args.resume_training:
    policy_net, value_net,  critic_net, running_state, running_reward = pickle.load(open('assets/learned_models/SIL/{}/{}_SIL_s{}.p'.format(args.dataset_size, args.env_name, args.seed), "rb"))
...
agent = Agent(env, policy_net, device, mean_action=args.use_mean,  running_state=running_state, render=args.render, num_threads=args.num_threads,)
def sil_step(batch, i_iter):
    states      = torch.from_numpy(np.stack(batch.state)).to(dtype).to(device)
    next_states = torch.from_numpy(np.stack(batch.state)).to(dtype).to(device) # Use for state-next_state distribution matching
    ...
    for _ in range(1):
        sampled_episodes = []
        epis = []
        for pair in range(len(masks)): # Split to episodes to do random matching (Corresponds to Step 3 of Algorithm 1)
        ...
        for trajectory in sampled_episodes:
            ...
            Y = torch.from_numpy(expert_traj[sample_traj_index]).to(dtype).to(device) # Randomly match (Corresponds to Step 3 of Algorithm 1)
            ...
            wasserstein_distance       = -(distance)                                         # Assign -wasserstein in order to GD to maximise if using adversary for training.
            ...
            if args.log_actual_sinkhorn:
                ...
                evaluation_transport_plan = optimal_transport_plan(X, Y, evaluation_cost_matrix, method='sinkhorn_gpu')
...
def sil():
    ...
    if args.resume_training ==True:
        R_avg  =  pickle.load(open('experiment-logs-sil/{}/{}/avg_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        R_max  =  pickle.load(open('experiment-logs-sil/{}/{}/max_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        R_min  =  pickle.load(open('experiment-logs-sil/{}/{}/min_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        sinkhorn_log =  pickle.load(open('experiment-logs-sil/{}/{}/skh_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), "rb"))
        episodic_rewards =  pickle.load(open('experiment-logs-sil/{}/{}/episodic_rewards_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), "rb"))
    ...
    for i_iter in range(args.max_iter_num):
        ...
        print(sum(episodic_rws)/len(episodic_rws))
        ...
        if args.save_model_interval > 0 and (i_iter) % args.save_model_interval == 0:
            ...
            pickle.dump((policy_net, value_net, critic_net, running_state, running_reward), open(os.path.join(assets_dir(), 'learned_models/SIL/{}/{}_SIL_s{}.p'.format(args.dataset_size, args.env_name, args.seed)), 'wb'))
            to_device(device, policy_net, value_net)

            pickle.dump(R_avg, open('experiment-logs-sil/{}/{}/avg_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))

            pickle.dump(R_max, open('experiment-logs-sil/{}/{}/max_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
 
            pickle.dump(R_min, open('experiment-logs-sil/{}/{}/min_seed{}.r'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
        
            pickle.dump(sinkhorn_log, open('experiment-logs-sil/{}/{}/skh_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
   
            pickle.dump(episodic_rewards, open('experiment-logs-sil/{}/{}/episodic_rewards_seed{}.l'.format(args.env_name, args.dataset_size, args.seed), 'wb'))
        torch.cuda.empty_cache()
        t1 = time.time()
